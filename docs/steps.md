...
a variable (`stepper`). This trial stepper will allow you to advance through the
trials. 
...
const stepper = api.useTrialStepper()
...
stepper.push(trials, {method: 'zip'})
...
stepper.push(trials, {method: 'outer'})
...
stepper.push(trials, {method: 'zip', autofill: true})
...
stepper.push(trials, {method: 'zip', shuffle: true})
...
stepper.push(trials, {method: 'zip', shuffle: true, repeat: 5})
...
- `stepper.index()`: Get the global index of the current trial
- `stepper.reset()`: Reset stepper back to the first trial

When the last step is reached, `stepper.next()` will call the callback function 
...
is met usually when `stepper.index()` is equal to the number of trials.
...
const num_iters = 20
stepper.push(trials, { shuffle: true, repeat: num_iters })
...
function chooseBandit(idx) {
  ...
  reward.value = stepper.value.arms[idx]
}
...
const stop = onKeyDown(keys, (e) => {
    ...
    if (stepper.value.index == num_iters - 1) {
      api.recordTrialData({
        trialnum: stepper.index(),
        arms: stepper.value.arms,
        choicenum: stepper.value.index,
    })
    }
...
This is the template code for the view. The `stepper.index()` method is
used to get the current step number, and `stepper.value` is used to access
the current trial data (e.g. `stepper.value.arms`).
...
<template>
  <div class="experiment">
    ...
    <div v-if="stepper.value">
      <h2>Trial {{ stepper.index() + 1 }} / {{ stepper.trials.length }}</h2>
      <p><strong>Arms:</strong> {{ stepper.value.arms.join(', ') }}</p>
