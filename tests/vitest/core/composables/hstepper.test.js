...
describe('useHStepper composable', () => {
  ...
  describe('append functionality', () => {
    ...
    it('should allow appending a single object', async () => {
      const stepper = getCurrentRouteStepper()
      const table = stepper.new()
      table.append({ color: 'red', shape: 'triangle' })
    ...
    it('should allow appending a single object', async () => {
      ...
      table.append([{ color: 'blue', shape: 'square' }])
      expect(table.rows).toHaveLength(2)
      expect(table.rows[1]).toEqual({ color: 'blue', shape: 'square' })

      // Should work with another single object
      table.append({ color: 'green', shape: 'circle' })
      expect(table.rows).toHaveLength(3)
      expect(table.rows[2]).toEqual({ color: 'green', shape: 'circle' })
    })
